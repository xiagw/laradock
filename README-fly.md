...
## 替换证书 Nginx SSL pem: $HOME/docker/laradock/nginx/ssl/default.pem
cp -vf $HOME/example.com.pem $HOME/docker/laradock/nginx/ssl/default.pem
...
cd $HOME/docker/laradock && docker compose exec -T mysql sh -c 'mysql -udefaultdb -p"$MYSQL_PASSWORD" defaultdb' < $HOME/.laradock/data/mysqlbak/db.sql
## mysql 进入命令行操作(本机)
...
- 备案原则：备案提供商和服务器提供商需一致
### 备案有效和无效示例说明
- 有效备案：阿里云服务器 + 阿里云备案（备案提供商和服务器提供商一致）
- 有效备案：腾讯云服务器 + 腾讯云备案（备案提供商和服务器提供商一致）
- 无效备案：非阿里云服务器 + 阿里云备案（备案提供商和服务器提供商不一致导致无效备案）
- 无效备案：非腾讯云服务器 + 腾讯云备案（备案提供商和服务器提供商不一致导致无效备案）
### 域名配置要求
- DNS解析：配置A记录指向服务器IP
